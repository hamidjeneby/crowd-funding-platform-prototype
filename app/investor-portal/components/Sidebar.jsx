...
import { LayoutDashboard, Wallet, Briefcase, Activity, Settings, Building2, Repeat, UserCheck, ShieldCheck } from "lucide-react";
...
export default function Sidebar() {
  ...
  const links = [
    { 
      ...
      match: (path) => path === "/investor-portal"
    },
    {
      name: "Portfolio",
      href: "#",
  ...
  return (
    <aside className="w-64 h-screen sticky top-0 bg-[#fdfbf7] border-r border-[#059669]/10 flex flex-col shadow-sm flex-shrink-0 z-40">
      <div className="p-6 overflow-y-auto">
        ...
        {/* Account Mode Indicator & Organization Switcher Section */}
        <div className="mb-6 p-3 rounded-lg bg-[#ecfdf5] border border-[#059669]/20 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            ...
            {organization ? (
              <span className="px-2 py-0.5 rounded bg-emerald-600 text-white font-semibold text-[10px] flex items-center gap-1">
                <Building2 className="w-3 h-3" /> Investor
              </span>
            ) : (
              <span className="px-2 py-0.5 rounded bg-blue-600 text-white font-semibold text-[10px] flex items-center gap-1">
                <UserCheck className="w-3 h-3" /> Investor
              </span>
            )}
          </div>
          ...
          <OrganizationSwitcher
            ...
            afterCreateOrganizationUrl="/investor-portal"
        ...
        <nav className="space-y-2">
          ...
          {/* Manage Organization Button (Enabled when active organization exists) */}
          {organization && (
            <button
              type="button"
